...
use App\Domain\Publishing\ValueObjects\SeoMetadata;
 ...
 * @property RichTextDocument|null $body
...
class Post extends Model
{
    ...
    /**
     * Publishing locks the slug and stamps the content-freshness signal the
     * first time it happens; republishing keeps the original timestamps.
     */
    public function publish(?User $editor, ?CarbonInterface $publishedAt = null): void
    {
        $this->status = PostStatus::Published;
        $this->scheduled_at = null;
        $this->published_at ??= $publishedAt ?? now();
        $this->slug_locked_at ??= now();
        $this->content_updated_at ??= now();
        $this->recordEdit($editor);
        $this->save();
    }

    public function schedule(CarbonInterface $scheduledAt, User $editor): void
    {
    ...
    public function unpublish(User $editor): void
    {
        $this->status = PostStatus::Draft;
        $this->scheduled_at = null;
        $this->featured_at = null;
    ...
    public function moveToTrash(User $editor): void
    {
        $this->status = PostStatus::Draft;
        $this->scheduled_at = null;
        $this->featured_at = null;
    ...
    {
        if (! $this->isPublished()) {
            throw new CannotFeatureUnpublishedPost($this);
        }

        $this->featured_at = now();
        self::withoutTimestamps(fn (): bool => $this->save());
    }
    ...
    private function recordEdit(?User $editor): void
    {
        $this->updated_by_id = $editor?->id;
        $this->lock_version++;
    }
    ...
    {
        return [
            'body' => RichTextDocumentCast::class,
